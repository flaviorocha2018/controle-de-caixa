...
export function Footer({ data }: Readonly<FooterProps>) {
  ...
  return (
    <div className="dark bg-white text-black py-16">
      <div className="container mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          {socialLink.map((link) => {
            return (
              <Link key={link.id} className="text-black hover:text-gray-300" href={link.url}>
      ...
      <p className="flex items-center justify-center"> Copyright ₢  {current_year}</p>
    </div>
  );
}
